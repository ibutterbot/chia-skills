...
use crate::schema::{PuzzleCandidate, PuzzleRecognition, WrapperInfo};
...
pub fn recognize_puzzle_and_solution(
    puzzle_reveal_bytes: &[u8],
    solution_bytes: &[u8],
) -> PuzzleRecognition {
...
fn try_cat_layer(
    ...
    Some(LayerMatch {
        ...
        params: json!({
            "asset_id": encode_hex_prefixed(layer.asset_id.as_ref()),
        }),
...
fn try_singleton_layer(
    allocator: &Allocator,
    puzzle: DriverPuzzle,
    solution: Option<NodePtr>,
) -> Option<LayerMatch> {
    let layer = SingletonLayer::<DriverPuzzle>::parse_puzzle(allocator, puzzle)
        .ok()
        .flatten()?;
    let mut parse_error = None;
    let (next_solution, solution_json) = match solution {
        Some(ptr) => match SingletonLayer::<DriverPuzzle>::parse_solution(allocator, ptr) {
            Ok(parsed) => (
                ...
                json!({
                    "status": "ok",
                    "inner_solution_tree_hash": node_tree_hash_hex(allocator, parsed.inner_solution),
                    "parsed_debug": format!("{parsed:?}"),
                }),
            ),
            Err(err) => {
...
fn try_did_layer(
    ...
    let layer = DidLayer::<NodePtr, DriverPuzzle>::parse_puzzle(allocator, puzzle)
        .ok()
        .flatten()?;
    let mut parse_error = None;
    let (next_solution, solution_json) = match solution {
        Some(ptr) => match DidLayer::<NodePtr, DriverPuzzle>::parse_solution(allocator, ptr) {
            Ok(DidSolution::Spend(inner_solution)) => (
                ...
                json!({
                    "status": "ok",
                    "kind": "spend",
                    "inner_solution_tree_hash": node_tree_hash_hex(allocator, inner_solution),
                }),
            ),
...
fn try_nft_state_layer(
    allocator: &Allocator,
    puzzle: DriverPuzzle,
    solution: Option<NodePtr>,
) -> Option<LayerMatch> {
    let layer = NftStateLayer::<NodePtr, DriverPuzzle>::parse_puzzle(allocator, puzzle)
        .ok()
        .flatten()?;
    let mut parse_error = None;
    let (next_solution, solution_json) = match solution {
...
fn try_nft_ownership_layer(
    ...
    let layer = NftOwnershipLayer::<DriverPuzzle, DriverPuzzle>::parse_puzzle(allocator, puzzle)
        .ok()
        .flatten()?;
    let mut parse_error = None;
    let (next_solution, solution_json) = match solution {
        Some(ptr) => {
            match NftOwnershipLayer::<DriverPuzzle, DriverPuzzle>::parse_solution(allocator, ptr) {
                Ok(parsed) => (
                    Some(parsed.inner_solution),
                    json!({
                        "status": "ok",
...
fn try_royalty_transfer_layer(allocator: &Allocator, puzzle: DriverPuzzle) -> Option<LayerMatch> {
    let layer = RoyaltyTransferLayer::parse_puzzle(allocator, puzzle)
        .ok()
        .flatten()?;
    Some(LayerMatch {
        ...
        solution: json!({
            "status": "unsupported",
            "message": "layer does not define a spendable solution parser",
        }),
        parse_error: None,
    })
}
...
fn try_augmented_condition_layer(
    ...
    let layer = AugmentedConditionLayer::<NodePtr, DriverPuzzle>::parse_puzzle(allocator, puzzle)
        .ok()
        .flatten()?;
    let mut parse_error = None;
    let (next_solution, solution_json) = match solution {
        Some(ptr) => {
            match AugmentedConditionLayer::<NodePtr, DriverPuzzle>::parse_solution(allocator, ptr) {
                Ok(parsed) => (
                    Some(parsed.inner_solution),
                    json!({
                        "status": "ok",
    ...
    Some(LayerMatch {
        ...
        params: json!({
            "condition_debug": format!("{:?}", layer.condition),
        }),
        next_puzzle: Some(layer.inner_puzzle),
        next_solution,
        solution: solution_json,
...
fn try_bulletin_layer(
    allocator: &Allocator,
    puzzle: DriverPuzzle,
    solution: Option<NodePtr>,
) -> Option<LayerMatch> {
    let layer = BulletinLayer::<DriverPuzzle>::parse_puzzle(allocator, puzzle)
        .ok()
        .flatten()?;
    let mut parse_error = None;
    let (next_solution, solution_json) = match solution {
...
fn try_option_contract_layer(
    ...
    let layer = OptionContractLayer::<DriverPuzzle>::parse_puzzle(allocator, puzzle)
        .ok()
        .flatten()?;
    let mut parse_error = None;
    let (next_solution, solution_json) = match solution {
...
fn try_revocation_layer(
    allocator: &Allocator,
    puzzle: DriverPuzzle,
    solution: Option<NodePtr>,
) -> Option<LayerMatch> {
    let layer = RevocationLayer::parse_puzzle(allocator, puzzle)
        .ok()
        .flatten()?;
    let mut parse_error = None;
    let (next_solution, solution_json) = match solution {
...
fn try_p2_singleton_layer(
    ...
    let layer = P2SingletonLayer::parse_puzzle(allocator, puzzle)
        .ok()
        .flatten()?;
    let mut parse_error = None;
    let (next_solution, solution_json) = match solution {
...
fn try_p2_curried_layer(
    ...
    Some(LayerMatch {
        ...
        params: json!({
            "puzzle_hash": encode_hex_prefixed(layer.puzzle_hash.as_ref()),
        }),
        next_puzzle: None,
        next_solution,
        solution: solution_json,
...
fn try_p2_one_of_many_layer(
    ...
    let layer = P2OneOfManyLayer::parse_puzzle(allocator, puzzle)
        .ok()
        .flatten()?;
    let mut parse_error = None;
    let (next_solution, solution_json) = match solution {
    ...
    Some(LayerMatch {
        ...
        params: json!({
            "merkle_root": encode_hex_prefixed(layer.merkle_root.as_ref()),
        }),
        next_puzzle: None,
        next_solution,
        solution: solution_json,
...
fn try_p2_delegated_conditions_layer(
    ...
    let layer = P2DelegatedConditionsLayer::parse_puzzle(allocator, puzzle)
        .ok()
        .flatten()?;
    let mut parse_error = None;
    let (next_solution, solution_json) = match solution {
        Some(ptr) => match P2DelegatedConditionsLayer::parse_solution(allocator, ptr) {
            Ok(parsed) => (
                ...
                json!({
                    "status": "ok",
                    "conditions_len": parsed.conditions.len(),
                    "parsed_debug": format!("{parsed:?}"),
                }),
            ),
            Err(err) => {
    ...
    Some(LayerMatch {
        ...
        params: json!({
            "public_key": encode_hex_prefixed(&layer.public_key.to_bytes()),
        }),
        next_puzzle: None,
        next_solution,
        solution: solution_json,
...
fn try_settlement_layer(
    ...
    let _layer = SettlementLayer::parse_puzzle(allocator, puzzle)
        .ok()
        .flatten()?;
    let mut parse_error = None;
    let (next_solution, solution_json) = match solution {
        Some(ptr) => match SettlementLayer::parse_solution(allocator, ptr) {
            Ok(parsed) => (
                ...
                json!({
                    "status": "ok",
                    "payments_len": parsed.notarized_payments.len(),
                    "parsed_debug": format!("{parsed:?}"),
                }),
            ),
            Err(err) => {
...
fn try_standard_layer(
    ...
    Some(LayerMatch {
        ...
        params: json!({
            "synthetic_key": encode_hex_prefixed(&layer.synthetic_key.to_bytes()),
        }),
        next_puzzle: None,
        next_solution,
        solution: solution_json,
...
fn candidate_from_match(matched: &LayerMatch, confidence: f64) -> PuzzleCandidate {
    PuzzleCandidate {
        ...
        source_path: Some(matched.source_path.to_string()),
        source_hash: Some(SOURCE_REF.to_string()),
    }
}
